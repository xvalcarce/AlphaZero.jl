...
const HParams = Dict("filters" => 128,
					 "blocks" => 5,
					 "policy_filters" => 32,
					 "value_filters" => 32,
					 ...
					 "n_games" => 2_000,
					 "mcts_n_iter" => 200,
					 "temperature" => PLSchedule([0,20,30,35], [1.0,1.0,0.6,0.2]),
					 "batch_size" => 1_024,
					 "learning_rate" => 1e-3
					 )
## Neural network

# NN type : residual network
...
self_play = SelfPlayParams(
	...
	mcts = MctsParams(
		...
		## Arena parameters : Neural Net (compare_networks) benchmarked on these games 
arena = ArenaParams(
	sim = SimParams(
	num_games = 256,
    	num_workers = 256,
...
params = Params(
	...
	[      0,        18],
  	[400_000, 1_000_000]))                                         # Size of the memory buffer, in terms of number of samples, per iterations

## Benchmark parameters
## Note : use config.jl to set DEPTH of circuit used for benchmarks
...
benchmark = [
	...
	SingleAudit(Benchmark.Full(arena.mcts), benchmark_sim), # mcts alone
	SingleAudit(Benchmark.NetworkOnly(;τ=1.0),benchmark_sim)] # network alone
